...
public interface IDashEvolutionWalletService
{
    ...
    /// <summary>
    /// Return a UNIQUE per-invoice shielded address at a freshly-allocated
    /// diversifier index (platform_wallet_manager_shielded_address_at —
    /// sansbankdao/platform branch shielded-address-at). All diversified
    /// indices share the account IVK, so the normal sync detects receipts to
    /// any index. Falls back to the shared default address
    /// (IsDiversified=false) when the loaded native library lacks the export.
    /// </summary>
    Task<DashEvolutionAddressResult> GetShieldedAddressAsync(
        DashEvolutionPaymentMethodConfig config,
        CancellationToken cancellationToken = default);

    /// <summary>
    /// Allocate a UNIQUE per-invoice Transparent (DIP-17 platform) receive
    /// address via platform_address_wallet_next_unused_receive_address,
    /// base58check-encoded (Dash P2PKH). Per-address BLAST balances give
    /// EXACT per-invoice attribution. Returns IsUnique=false with a null
    /// Address when unavailable (stale .so, provider down) — the invoice
    /// is then shielded-only and the checkout hides the Transparent tab.
    /// </summary>
    Task<DashEvolutionTransparentAddressResult> GetTransparentAddressAsync(
        DashEvolutionPaymentMethodConfig config,
        CancellationToken cancellationToken = default);
}

/// <summary>
/// Result of a per-invoice shielded address allocation.
/// </summary>
public sealed class DashEvolutionAddressResult
{
    /// <summary>The bech32m-encoded shielded address the customer pays to.</summary>
    public string Address { get; set; } = string.Empty;

    /// <summary>Diversifier index used (0 = shared default address).</summary>
    public uint DiversifierIndex { get; set; }

    /// <summary>
    /// True when a UNIQUE per-invoice diversified address was allocated (safe
    /// to add to TrackedDestinations); false when the shared default address
    /// was returned (fallback — MUST NOT be tracked: the AddressInvoices PK
    /// (Address, PaymentMethodId) would collide on the second invoice).
    /// </summary>
    public bool IsDiversified { get; set; }
}

/// <summary>
/// Result of a per-invoice Transparent (DIP-17 platform) address allocation.
/// </summary>
public sealed class DashEvolutionTransparentAddressResult
{
    /// <summary>The base58check-encoded Dash P2PKH address the customer pays
    /// to ('X…' mainnet / 'y…' testnet). Null when unavailable.</summary>
    public string? Address { get; set; }

    /// <summary>True when a fresh unique address was allocated (safe to add
    /// to TrackedDestinations); false when Transparent receive is
    /// unavailable for this invoice.</summary>
    public bool IsUnique { get; set; }

    public static DashEvolutionTransparentAddressResult Unavailable() => new() { Address = null, IsUnique = false };
}
...
public class DashEvolutionPaymentPromptDetails
{
    ...
    /// <summary>Orchard diversifier index of the address. Null when the
    /// shared default address (index 0) was used as a fallback (stale .so
    /// without shielded_address_at).</summary>
    public uint? DiversifierIndex { get; set; }
    ...
    /// emits both when Transparent receive is available (the customer picks
    /// the transport in the checkout; the prompt Destination is the default
    /// tab's address).</summary>
    public bool Shielded { get; set; } = true;

    /// <summary>The base58check-encoded Transparent (DIP-17 platform)
    /// receive address for this invoice, when allocated. Null when
    /// Transparent receive was unavailable (stale .so / provider down) —
    /// the checkout then shows only the Shielded tab.</summary>
...
public class DashEvolutionPaymentMethodHandler : IPaymentMethodHandler
{
    ...
    {
        public Task<DashEvolutionAddressResult> AddressTask;
        public Task<DashEvolutionTransparentAddressResult> TransparentAddressTask;
    ...
    public Task BeforeFetchingRates(PaymentMethodContext paymentMethodContext)
    {
        ...
        if (paymentMethodContext.Prompt.Activated)
        {
            ...
            {
                AddressTask = _walletService.GetShieldedAddressAsync(config,
                    CancellationToken.None),
                TransparentAddressTask = _walletService.GetTransparentAddressAsync(config,
                    CancellationToken.None),
                Config = config
            };
        }
    ...
    public async Task ConfigurePrompt(PaymentMethodContext paymentContext)
    {
        ...
        DashEvolutionAddressResult allocated;
        try
        {
            allocated = await prepare.AddressTask;
        }
        catch (Exception ex)
        {
            throw new PaymentMethodUnavailableException(
        ...
        // Transparent (DIP-17) allocation is best-effort: a failure or an
        // unavailable result never breaks invoice creation — the invoice
        // just becomes shielded-only.
        DashEvolutionTransparentAddressResult transparent;
        try
        {
            transparent = await prepare.TransparentAddressTask;
        }
        catch (Exception)
        {
            transparent = DashEvolutionTransparentAddressResult.Unavailable();
        }
        ...
        // With per-invoice diversified addresses (shielded_address_at FFI,
        // sansbankdao/platform branch shielded-address-at) each invoice gets a
        // UNIQUE address, so the AddressInvoices row (PK = (Address,
        // PaymentMethodId)) never collides and the destination can be tracked
        // like BitcoinLikePaymentHandler does. When the native library lacks
        // the export (stale .so), the wallet service falls back to the shared
        // default address (IsDiversified=false) — tracking THAT would hit
        ...
        // ON CONFLICT) on the second invoice, so it stays disabled. The sync
        // service correlates payments by balance-delta either way (see
        // DashEvolutionSyncService header).
        if (allocated.IsDiversified)
            paymentContext.TrackedDestinations.Add(address);

        // Unique per-invoice Transparent address → tracked like any
        // BitcoinLike destination (the AddressInvoices PK never collides).
        // The sync service matches per-address BLAST balance deltas against
        // this row for EXACT invoice attribution.
        if (transparent.IsUnique && transparent.Address is not null)
            paymentContext.TrackedDestinations.Add(transparent.Address);

        paymentMethod.Details = JObject.FromObject(new DashEvolutionPaymentPromptDetails
        {
            Address = address,
            AccountIndex = config.AccountIndex,
            DiversifierIndex = allocated.IsDiversified ? allocated.DiversifierIndex : null,
            TransparentAddress = transparent.IsUnique ? transparent.Address : null,
            Shielded = true
        }, Serializer);

        // Checkout default tab: Destination drives the QR the customer sees
        // first (and the pay-in-wallet URI). Prefer Transparent when BOTH
        // transports have per-invoice unique addresses (Transparent matches
        // what most Dash wallets send natively and attributes exactly);
        // otherwise stay on the shielded/default destination.
        if (transparent.IsUnique && transparent.Address is not null && allocated.IsDiversified)
            paymentMethod.Destination = transparent.Address;
    }
