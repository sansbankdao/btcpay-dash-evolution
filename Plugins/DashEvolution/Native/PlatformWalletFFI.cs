...
public static class PlatformWalletFFI
{
    ...
    /// NOTE: for per-invoice diversified addresses use ShieldedAddressAt
    /// (added in the sansbankdao/platform fork, branch shielded-address-at).
    /// </summary>
    ...
    /// <summary>
    /// platform_wallet_manager_shielded_address_at (sansbankdao/platform
    /// branch shielded-address-at): the Orchard payment address at
    /// diversifier index `index` for `account` — same semantics as
    /// ShieldedDefaultAddress (which is index 0). All diversified indices
    /// share the account's incoming viewing key, so the normal sync detects
    /// receipts to any index. Requires the patched
    /// libplatform_wallet_ffi.so (the upstream .so has no such export).
    /// </summary>
    [DllImport(LibName, EntryPoint = "platform_wallet_manager_shielded_address_at")]
    public static extern PlatformWalletFFIResult ShieldedAddressAt(
        ulong handle,
        byte[] walletIdBytes,            // *const u8, 32 bytes
        uint account,
        uint index,                      // diversifier index
        byte[] outBytes43,               // *mut u8, 43 writable bytes
        [MarshalAs(UnmanagedType.U1)] out bool outPresent);
