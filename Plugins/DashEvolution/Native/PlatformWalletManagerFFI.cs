...
/// <summary>
/// Callback for a completed platform-address (BLAST) sync pass. Delivers a
/// const array of PlatformAddressSyncWalletResultFFI (one per wallet with a
/// platform-address provider) + the pass's Unix timestamp.
/// (event_handler.rs:117 — on_platform_address_sync_completed_fn)
/// </summary>
[UnmanagedFunctionPointer(CallingConvention.Cdecl)]
public delegate void PlatformAddressSyncCompletedCallback(
    IntPtr ctx,
    IntPtr results,           // *const PlatformAddressSyncWalletResultFFI
    UIntPtr count,
    ulong syncUnixSeconds);
