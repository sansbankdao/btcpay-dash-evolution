...
public static class Bech32m
{
    ...
    /// <summary>
    /// Encode a DIP-0018 Transparent (Platform) address: bech32m with HRP
    /// "dash"/"tdash" and data = typeByte || hash160 (21 bytes).
    /// Type bytes: 0xb0 = P2PKH, 0x80 = P2SH (user-facing bech32m bytes,
    /// NOT the 0x00/0x01 GroveDB storage bytes). Source: rs-dpp
    /// address_funds/platform_address.rs to_bech32m_string (DIP-0018).
    /// </summary>
    /// <param name="mainnet">true for HRP "dash", false for "tdash".</param>
    /// <param name="typeByte">0xb0 for P2PKH, 0x80 for P2SH.</param>
    /// <param name="hash20">20-byte address hash (hash160).</param>
    public static string EncodePlatformAddress(bool mainnet, byte typeByte, byte[] hash20)
    {
        if (hash20 == null || hash20.Length != 20)
            throw new ArgumentException("Platform address hash must be exactly 20 bytes", nameof(hash20));
        var hrp = mainnet ? "dash" : "tdash";
        // 1 type byte + 20 hash bytes = 21 bytes payload
