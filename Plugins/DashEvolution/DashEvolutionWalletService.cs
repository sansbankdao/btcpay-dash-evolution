...
public class DashEvolutionWalletService : IDashEvolutionWalletService
{
    ...
    public Task<DashEvolutionAddressResult> GetShieldedAddressAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        // Placeholder parity with GetShieldedDefaultAddressAsync. NOT
        // diversified (the same constant address every call) so the handler
        // keeps TrackedDestinations disabled — same as the live service's
        // stale-.so fallback path.
    ...
    /// <summary>
    /// Placeholder parity for the new interface member: the stub has no
    /// Transparent (DIP-17) backend, so it always reports unavailable —
    /// invoices become shielded-only.
    /// </summary>
